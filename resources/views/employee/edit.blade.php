            ...
            <div class="col-md-8 col-md-offset-2"> 
                <div class="panel rounded shadow">
                    ...
                    <div class="panel-body no-padding">
                        ...
                        <form class="form-horizontal" id="updateRecord" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="form-body">
                                <div class="form-group no-margin">
                                    <div class="row">
                                        ...
                                        <label class="control-label">Employee First Name </label>
                                            <input type="text" class="form-control mb-15" value="{{ $employee->firstname }}" name="firstname" placeholder="Employee First Name"/ value="">
                                        </div>
                                        <div class="col-md-6">
                                        <label class="control-label">Employee Last Name </label>
                                            <input type="text" class="form-control mb-15" value="{{ $employee->lastname }}" name="lastname" placeholder="Employee Last Name"/>
                                        </div>
                                    </div>
                                    <div class="row">
                                        <div class="col-md-6">
                                        <label class="control-label">Employee Email</label>
                                            <input type="email" class="form-control mb-15" value="{{ $employee->email }}" name="email" placeholder="Employee Email"/ >
                                        </div>
                                        <div class="col-md-6">
                                        ...
                                        @foreach ($Comapny as $value)
                                                <option value="{{$value->id}}" @if( $value->id == $employee->company) selected @endif>{{$value->name}}</option>
                                        @endforeach
                                        @endif
                                        </select>
                                        </div>
...
@push('scripts')
<script>
    $('#updateRecord').on('submit', function(event){
